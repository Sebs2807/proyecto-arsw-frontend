...
import AssistantSidebar from "./AgentsSidebar";
...
const Sidebar: React.FC = () => {
  ...
  const navButtons = [
    ...
    },
    {
      ...
      sideMenuComponent: <AssistantSidebar />,
  ...
  return (
    <div className="h-full flex flex-col flex-1">
      ...
      <div className="flex flex-col flex-1 bg-dark-600 rounded-xl p-2 min-h-0">
        <p className="text-lg font-semibold text-text-primary text-center">
          {activeNav?.text}
        </p>
        <div className="flex-1 overflow-hidden rounded-xl">
          {activeNav?.sideMenuComponent}
        </div>
      </div>
    </div>
  );
